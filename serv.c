...
int                 x_log, dbg;  /* the file-descriptors          */
long                t;           /* time                          */
...
int
fill(text, size, val) char *text, val;
int size;
{
  while (size--)
    {
...
int
blankstrip(text, size) char *text;
int size;
{
  while (size--)
    {
...
/*VARARGS*/
int
to_log(p1, p2, p3, p4, p5, p6, p7, p8, p9) char *p1, *p2, *p3, *p4, *p5, *p6,
*p7, *p8, *p9;
{
  char buf[512];
  if (logfile || dbgfile)
...
/*VARARGS*/
int
to_dbg(p1, p2, p3, p4, p5, p6, p7, p8, p9) char *p1, *p2, *p3, *p4, *p5, *p6,
*p7, *p8, *p9;
{
  char buf[512];
  if (dbgfile)
...
void
check_free(ch) long ch;
{ /* check for pending passwds */
  int k, n;
  for (k = n = 0; k < NUSERS; k++)
    {
      if (users[k].us_flag && users[k].us_channel == ch)
        {
          n++;
        }
    }
...
void
write_msg(person, channel, text) int person;
long   channel;
char * text;
{
  int ret_code;
  if (person < 0 || person >= NUSERS)
    {
      return;
    }

  if (!users[person].us_flag)
    {
      return;
    }
  ...
  if (channel >= 0 && channel != users[person].us_channel)
    {
      return;
    }
...
char *argv[];
{
  ...
  if (errcnt)
    {
      bad("usage: serv [-d debugfile] [-l logfile]\n");
    }
  ...
  if (dbgfile)
    {
      ...
      if (dbg < 0)
        {
          bad("cannot open dbgfile.\n");
        }
    }

  if (logfile)
    {
      x_log = open(logfile, O_WRONLY | O_APPEND | O_CREAT, 0600);
      if (x_log < 0)
        {
          bad("cannot open logfile.\n");
        }
    }
  ...
  for (i = 0; i < NUSERS; i++)
    {
      users[i].us_flag = 0, chans[i].ch_chan = -1L;
    }
    ...
    case UMAGIC:  /* user text */
      ...
      switch (magic)
        ...
        case UMAGIC1: /* user first time call */
          ...
          t                 = time((long *)0);
          to_log(
            "Incoming Call From %s At %24.24s\n",
            users[i].us_from,
            ctime(&t));
          sprintf(buf, "+++%s+++\n", users[i].us_nickname);
          to_log("%s", buf);
          ...
          if (!strncmp(cp.cp_text, ".p ", 3))
            {
              ...
              if (!users[i].us_channel)
                {
                  write_msg(i, -1L, "Channel 0 cannot be protected.\n");
                  break;
                }
              ...
              for (k = n = 0; k < NUSERS; k++)
                {
                  if (users[k].us_flag
                      && users[k].us_channel == users[i].us_channel)
                    {
                      n++;
                    }
                }
              ...
              if (n > 1)
                {
                  write_msg(i, -1L, "You are not alone on the channel.\n");
                  break;
                }
          ...
          if (!strncmp(cp.cp_text, ".c", 2))
            {
              ...
              for (k = 0; k < NUSERS; k++)
                {
                  write_msg(k, old, buf), write_msg(k, new, buf);
                }
