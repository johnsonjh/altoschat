 ...
 *
 *      The following comes from the original author
 *      (korn@altger) of this file:
 * --
 ...
 *      May 8, 1995 for Linux and Internet. I have also added a dirty
 *      who_am_i() in order to get the rlogin hostname for the list.
 *      It is ugly, I know.
 */
      ...
      if (!strncmp(cp.cp_text, ".so", 3))
        { /* source from a file. */
          ...
          if (sfd < 0)
            ...
            {
              process(sfd), close(sfd), msg("done.\n");
            }
...
main()
{
  ...
  if (child < 0)
    {
      bad("cannot create second process.\n");
    }
  ...
  if (!child) /* in the child-process:                    */
  ...
  else /* in the parent */
    {
      ...
      alarm(3);
      pause();                               /* give child time to open pipe */
...
get_pipes()
{
  ...
  umask(0);
  if (mknod(pfrom, 010400, 0))
    {
      bad("can't create answer-pipe.\n");
    }
}
...
get_user()
{
  ...
  if (pw)
    {
      strncpy(logname, pw->pw_name, 8);
    }
  else
    {
      sprintf(logname, "%-8d", geteuid());
    }
  ...
  if (log)
    {
      strncpy(nickname, log, 32);
    }
  else
    {
      sprintf(nickname, "%-.32s", logname);
    }
  ...
  if (frm)
    ...
    {
      strcpy(from, "somewhere");
    }

  if (from[0] == ':')
    {
      strcpy(from, "Xlocalhost");
    }

  if (from[0] == '\0')
    {
      strcpy(from, "localhost");
    }
...
/*
 * Quick&Dirty who_am_i  patched from GNU sh_utils and cleaned up a lot by
 * venta@i2ack This code goes under the usual GNU-GPL (i think). You will find
 * GNU Copyrights all around your filesystem, since you are running Linux!
